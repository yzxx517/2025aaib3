...
public:
    string multiply(string num1, string num2) {
        if(num1=="0" || num2=="0") return "0"; ///0乘上任何數都變成0

        int N1 = num1.length(), N2 = num2.length();
        int N = N1 + N2;
        vector<int> ans(N); ///伸縮自如的陣列,長度N
        for(int i=N1-1; i>=0; i--){ //逐項相乘 , 放入 ans裡面
        ...
        string strAns = ""; /// 一開始 字串裡面 甚麼都沒有
        if(ans[0]==0){  ///首相是0, 就避開他 不要一開始就在字串放0
            for(int i=1; i <N; i++) strAns += (char)(ans[i]+'0');
        }else{  /// 首相不是0, 很好 就從頭到尾都加到字串裡
